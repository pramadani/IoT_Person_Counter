...
def update_temp_df(interval, temperature):
    ...
    if current_time - last_temp_update >= interval:
        ...
        fig_temp.data = []
        fig_temp.add_trace(go.Scatter(x=df.index, y=df["Temperature"], mode='lines+markers', name='Temperature'))
...
def update_count_df(interval, person_count):
    ...
    if current_time - last_person_update >= interval:
        ...
        fig_person.data = []
        fig_person.add_trace(go.Scatter(x=df.index, y=df["Person Count"], mode='lines+markers', name='Person Count'))
